 ...
 * (https://portal.example.com/) is Cloudflare Access-protected — every request
 ...
 *     past).
 *  3. Forwards the request to the PORTAL_WORKER service binding
 *     with X-User-Email set from the verified `email` claim.
 *
 * Bindings (configured on the Pages project — Settings → Functions):
 *   PORTAL_WORKER — Service binding to the fishbowl-portal-worker Worker.
 *
 ...
 */
const ACCESS_TEAM_DOMAIN = 'https://your-team.cloudflareaccess.com';
const ACCESS_APP_AUD     = '<YOUR_ACCESS_APP_AUD>';
...
export async function onRequest(context) {
    ...
    if (!jwt) {
        return new Response(JSON.stringify({
            error: 'Not authenticated via Cloudflare Access — the Fishbowl BI portal must be loaded through https://portal.example.com.',
    ...
    if (!env.PORTAL_WORKER) {
        return new Response(JSON.stringify({
            error: 'Service binding PORTAL_WORKER not configured on Pages project. Add it under Pages → Settings → Functions → Service bindings.',
    ...
    return env.PORTAL_WORKER.fetch(new Request(request.url, init));
}
