/**
 * Cloudflare Worker — Fishbowl REST API proxy for the Fishbowl BI portal.
 *
 * Architecture (post-Access migration):
 *   Staff browser (logged in via Cloudflare Access OTP on portal.example.com)
 *     → Pages Function /api/* on portal.example.com (same-origin)
 ...
 *                    e.g. https://fb-api.example.com  (NOT localhost —
...
//      domain wildcard @example.com and the global *):
...
async function loadActiveConnection(env, email) {
    ...
    if (Array.isArray(entry.connections)) {
        ...
        return {
            ...
            fbPassword,
            appName:     conn.appName || 'Fishbowl BI Portal',
    ...
    return {
        ...
        fbPassword,
        appName:     entry.appName || 'Fishbowl BI Portal',
...
export class TokenManager {
    ...
    async fbLogin(creds) {
        ...
        const resp = await fetch(server.replace(/\/$/, '') + '/api/login', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                appName: creds.appName || 'Fishbowl BI Portal',
...
async function handleConnect(env, email, request) {
    ...
    try {
        ...
        if (!loginResp.ok) {
            ...
            // to approve "Fishbowl BI Portal" in Maintenance → Integrated
            ...
            if (/integrated application has not been approved|not yet been approved|has not been approved/i.test(respText)) {
                return jsonResponse(401, {
                    error: 'This Fishbowl tenant has not yet approved the Fishbowl BI integrated application. Your Fishbowl administrator needs to approve "Fishbowl BI Portal" (app ID 102) under Maintenance → Integrated Applications. Once approved, try connecting again.',
                });
            }
            return jsonResponse(401, { error: CONNECT_GENERIC_FAIL });
        }
    ...
    async fetch(request, env, ctx) {
        ...
        if (!conn) {
            return jsonResponse(403, {
                error: 'No Fishbowl account is connected for ' + email + '. Sign in to your Fishbowl account from the Fishbowl BI page.',
                needsLogin: true,
            });
        }
        ...
        // account (e.g. all @example.com → fbUsername "shared-fb") land on the
